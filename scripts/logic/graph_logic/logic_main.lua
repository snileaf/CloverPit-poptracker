...
cloverpit_location = {}
cloverpit_location.__index = cloverpit_location
...
---@type table<string, cloverpit_new_return>
NAMED_LOCATIONS = {}
local stale = true
...
function cloverpit_location.new(name)
    local self = setmetatable({}, cloverpit_location)
    if name then
...
---@param rule? function
function cloverpit_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = cloverpit_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
---@param exit string|alttp_location_new_return alttp_location_new_return or code/name
---@param rule? function
function cloverpit_location:connect_two_ways(exit, rule)
...
---@param exit string|alttp_location_new_return alttp_location_new_return or code/name
---@param rule? function
function cloverpit_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
---@param exit string|alttp_location_new_return alttp_location_new_return or code/name
---@param rule? function
function cloverpit_location:connect_two_ways_entrance(name, exit, rule)
...
function cloverpit_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
---@param rule1? function
---@param rule2? function
function cloverpit_location:connect_two_ways_stuck(exit, rule1, rule2)
...
function cloverpit_location:accessibility()
...
---@param keys integer
function cloverpit_location:discover(accessibility, keys)
...
Entry_point = cloverpit_location.new("Entry_point")
